...
sub rrc_freq_cell_info {
	...
	if ( $tag->has('MSG_ID_RRC_FREQ_CELL_INFO_PRINT') ) {
		...
		foreach my $freq_item ($xml->findnodes('//local_params/freq_info_print/ElemNode')) {	
			if ( (my $uarfcn = $freq_item->findvalue('./uarfcn')) ne "0xffff") {
				...
				for( my $i=0; $i < $no_cell; ++$i ) {
					...
					if (hex($rscp) > 0) {
						#push @freq_info, $uarfcn;
						#$tag->add("uarfcn[$i]" , $uarfcn);
						my $cell_param_id = $cell_item[$i]->findvalue('./cell_param_id');
						push @freq_info, $uarfcn, $cell_param_id, $rscp;
						#$tag->add("cell_param_id[$i]" , $cell_param_id);
						#$tag->add("rscp[$i]" , $rscp);
					}
				}
			}
		...
		if ( @freq_info ) {
			$tag->add( "freq_info", join(" ", @freq_info ) );
		}
	}
}
...
sub rrc_con_setup {
	my $xml = shift;
	my $tag = shift;
	if ( $tag->has('MSG_ID_RRCA_AS_RLC_DATA_IND') ) {
		my $rrc_StateIndicator = $xml->findvalue('//peer_params/message//rrc_StateIndicator');
		if( $rrc_StateIndicator) {
			$tag->add("rrc_StateIndicator" , $rrc_StateIndicator);
		}
	}
...
sub cell_camp_rsp {
	my $xml = shift;
	my $tag = shift; ho_status
	if ( $tag->has('MSG_ID_DM_RAT_CHG_TO_GSM_CNF') ) {
		my $target_state = $xml->findvalue('//local_params/target_state');
		$tag->add("target_state" , $target_state);
...
sub get_next_xml {
	...
	my $data;
	my $n;
	while ( ($n = read $file_id, $data, 10240) != 0 ) {
		$store .= $data;
		if ( $store =~ s!.*?($beg.*?$end)!!sm ) { #remove the matched xml from store
